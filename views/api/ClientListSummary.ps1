if($operation -eq "ClientListSummary_submit" -and $(Test-scupPSRole -Name "helpdesk" -User $WebEvent.authenticatedUser) -and ($collection = $WebEvent.Query.submitcollection) -and ($collection -in $((Get-scupPSValue -Name "Collection_BrowsingAllowed").Split(";")))){
